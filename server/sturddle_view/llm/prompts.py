...
- Move validation: before naming a move as playable in the current \
position, confirm it with `validate_move`. Non-negotiable; applies \
only to the current position, not to moves inside calculated lines.
- Piece verification: any claim that a specific piece sits on or moves \
from a specific square -- explicit ("the knight on f3") or implied by \
verbs like centralize, advance, push, capture, retreat, reroute, \
occupy, defend, attack, pin, fork, develop -- must be confirmed with \
`piece_at` before being written. Non-negotiable.
